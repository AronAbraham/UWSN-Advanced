...
const SensorNode = React.forwardRef(({ nodeId, position, color, isActive, isSimulating, energy, waterCurrent, isMPR, viewMode }, ref) => {
  ...
  const nodeColor = isActive
    ? "#00ff00" // Razer green for active nodes
    : energy > 60
      ? "#00ff00" // Razer green for high energy
      : energy > 30
        ? "#88ff88" // Lighter green for medium energy
        : "#00aa00"; // Darker green for low energy

  const nodeMaterial = isActive
    ? <meshStandardMaterial color={nodeColor} emissive={nodeColor} emissiveIntensity={0.7} />
    : <meshStandardMaterial color={nodeColor} emissive={nodeColor} emissiveIntensity={0.3} />;
  ...
  return (
    <group position={[0, 0, 0]} ref={ref}>
      ...
      <group ref={groupRef} position={pos}>
        ...
        <EnergyIndicator energy={energy} isActive={isActive} />
...
// Energy indicator
const EnergyIndicator = ({ energy, isActive }) => {
  return (
    <group position={[0, 2.5, 0]} rotation={[0, 0, 0]}>
      <mesh>
        <torusGeometry args={[3, 0.4, 16, 32, Math.PI * 2 * (energy / 100)]} />
        <meshStandardMaterial
          color={isActive ? "#00ff00" : "#003300"}
          emissive={isActive ? "#00ff00" : "#003300"}
  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, 10, 0]}>
        ...
        <meshStandardMaterial color="#222222" emissive="#00ff00" emissiveIntensity={0.5} />
      </mesh>
      <Text
        position={[0, 20, 0]}
        color="#00ff00"
...
const CommunicationPath = ({ startPos, endPos, protocol }) => {
  // Different protocols have different visualization styles but all with Razer theme
  const getPathColor = () => {
    switch (protocol) {
      case 'VBF': return '#00ff00'; // Razer green
      case 'HHVBF': return '#00dd00'; // Slightly darker green
      case 'DBR': return '#00bb00'; // Darker green
      case 'EEDBR': return '#00aa00'; // Even darker green
      case 'OLSR': return '#009900'; // Darkest green
  ...
  return (
    <line>
      ...
      <lineBasicMaterial attach="material" color={getPathColor()} opacity={0.5} transparent />
    </line>
  );
};
...
const Packet = ({ startPos, endPos, progress, protocol, isControl, sourceNodeId, targetNodeId, nodeRefs }) => {
  // Use Razer green for packets
  const packetColor = isControl
    ? '#00dd00' // Slightly different green for control packets
    : '#00ff00'; // Razer green for data packets
